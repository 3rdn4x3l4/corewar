...
/*   Updated: 2020/06/29 15:22:57 by alagache         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	fill_cor(t_file *file, int counter)
{
	int		iterator;
	int		cor_iter;
	...
	if (counter == 0)
	{
		ft_dprintf(STDERR_FILENO, EMPTY_CODE);
		free(file->cor);
		return (FAILURE);
	}
	iterator = 0;
...
int			parse_op(t_file *file)
{
	...
	if (file->cor == NULL)
		return (FAILURE);
	if (fill_cor(file, counter) == FAILURE)
		return (FAILURE);
	return (SUCCESS);
}
