...
/*   Updated: 2020/06/29 20:43:45 by alagache         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	parse_header(t_file *file)
{
	...
	if (whitespace_header(file) == FAILURE)
		return (FAILURE);
	if (file->head_name != 1 || file->head_comment != 1)
		return (WRONG_HEADER);
	return (SUCCESS);
}
...
static int	handle_parse_error(int ret, t_file *file)
{
	...
	if (ret == WRONG_HEADER)
		ft_dprintf(STDERR_FILENO, HEADER_STR);
